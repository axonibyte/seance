...
send (EACCES) while the bridge is still settling, and the kernel demotes by
`net.inet.carp.senderr_demotion_factor` per failed send — then the node goes
BACKUP and never re-sends to unwind it, so it advertises its own identity at a
losing skew and cannot reclaim its vhid until the counter is reset (decisions
D-192, D-193). Two independent guards, apply both:

```sh
# 1. Cure it at the source: a failed send demotes by ZERO. But the ORDER matters
#    (D-193): rc.d/sysctl runs long before rc.d/kld, so if carp is loaded from
#    kld_list the sysctl is set against an oid that does not exist yet and is
#    silently dropped -- and senderr_demotion_factor is NOT a loader tunable, so
#    loader.conf cannot set it directly. Load carp from the LOADER instead, so it
#    exists when rc.d/sysctl runs, before netif brings the vhids up:
sysrc -f /boot/loader.conf carp_load=YES     # carp is up before rc.d/sysctl
sysrc kld_list-="carp"                        # and no longer loaded twice
sysrc -f /etc/sysctl.conf net.inet.carp.senderr_demotion_factor=0
sysctl net.inet.carp.senderr_demotion_factor=0   # for the running kernel too
#    (a kernel with carp compiled in needs no carp_load; ifdown demotion, a real
#    link loss, is untouched, and `seance verify` still names any demotion.)
...
`seance carp-repair --boot` after the network — and `syslogd` (D-193: an earlier
version ran before syslogd and logged into the void) — are up: it waits for the
vhid interfaces to settle, then converges a *stale* demotion to 0, and **refuses**
a demotion that is *true* (a vhid interface still down), because clearing that
would advertise a fitness the node does not have. It is the one place seance
writes live CARP state (design §1). Its verdict lands in
`/var/log/seance-carp-reconcile.log` (durable, even if syslog was not up) and in
syslog (`daemon.notice`, tag `seance`). Run `seance carp-repair` by hand any
time — `--check` reports, `--fix` reconciles.
